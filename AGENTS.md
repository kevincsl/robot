...
| Skill | 類型 | 觸發方式 | 腳本 / 工具 |
|-------|------|---------|------------|
| **chromeontg** | 本機腳本 | 手動 | `scripts/skills/chrome_tg_runner.py` |
| **imagegen** | 本機腳本 | 手動 | `scripts/skills/pollinations_imagegen.py` + Gemini CLI |
| **pollinations-imagegen** | 本機腳本 | 手動 | `scripts/skills/pollinations_imagegen.py` |
| **gemini-imagegen** | 本機腳本 | 手動 | `gemini -p "..." --yolo` |
| **videogen** | 本機腳本 | 手動 | `scripts/skills/pollinations_videogen.py` + Gemini CLI |
| **image-edit-loop** | 本機腳本 | 手動 | `scripts/skills/image_edit_loop.py` |
| **send-tg-file** | 本機腳本 | 手動 | `scripts/skills/send_tg_file.py` |
| **compress-context** | 本機腳本 | 手動 / 自動（token 超限） | `scripts/skills/compress_context.py` |
| **sendmail** | MCP 工具 | 手動（寄信、email） | `mcp__sendmail__sendmail` |
| **update-config** | 系統內建 | 手動（設定/權限/hooks） | — |
| **keybindings-help** | 系統內建 | 手動（自訂快捷鍵） | — |
| **simplify** | 系統內建 | 手動（審查/簡化程式碼） | — |
| **fewer-permission-prompts** | 系統內建 | 手動 | — |
| **loop** | 系統內建 | 手動（重複執行任務） | — |
| **schedule** | 系統內建 | 手動（排程任務） | — |
| **claude-api** | 系統內建 | **自動**（import anthropic / Anthropic SDK） | — |
| **init** | 系統內建 | 手動（初始化 CLAUDE.md） | — |
| **review** | 系統內建 | 手動（審查 PR） | — |
| **security-review** | 系統內建 | 手動（安全性審查） | — |

> 詳細用法見下方各 Skill 章節。本機腳本皆位於 `scripts/skills/`。
...
## compress-context — LLMLingua Prompt Compression + Coco Index
...
- If over token limit: runs LLMLingua semantic compression → falls back to truncating oldest messages if still over limit
- Maintains `files/coco_index.json` — a rolling structured index of key facts (tool calls, errors, file paths) extracted from the conversation, so nothing critical is lost
- Preserves system message (index 0) at all times
...
**Typical workflow in robot/main:**
1. Before sending to API: `python scripts/skills/compress_context.py --input files/conversation.json --output files/conversation_compressed.json`
2. If still over limit after compression: increase `--ratio` (e.g. `0.3`) or lower `--token-limit`
3. Use `files/conversation_compressed.json` as the input to the API call

**Coco index (`files/coco_index.json`):**
- `facts[]` — last 50 extracted facts (tool calls, errors, file paths)
- `summary` — human-readable compression log
- `updated_at` — ISO timestamp
- Survives truncation: key decisions and errors are preserved even after oldest messages are dropped

**Dependencies:** `pip install llmlingua tiktoken`
